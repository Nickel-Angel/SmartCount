...
public class FeedViewModel extends ViewModel {
    ...
    public final MutableLiveData<Comment> mFeedComment;
    ...
    public void addEvaluate(String feedId, String toUid, String comment) {
        OkUtil.post()
                ...
                .addParam("type", "0")
                .execute(new ResultCallback<Result<Comment>>() {
                    @Override
                    public void onSuccess(Result<Comment> response) {
                        String code = response.getCode();
                        if (ResultConstant.CODE_SUCCESS.equals(code)) {
                            Comment comment = response.getData();
                            comment.setType(0);
                            mFeedComment.postValue(comment);
                        } else {
        ...
        OkUtil.post()
                ...
                .execute(new ResultCallback<Result<Comment>>() {
                    ...
                    public void onSuccess(Result<Comment> response) {
                        String code = response.getCode();
                        if (ResultConstant.CODE_SUCCESS.equals(code)) {
//                            mFeedComment.postValue(response.getData());
                            Comment comment = response.getData();
                            comment.setType(1);
                            mFeedComment.postValue(comment);
                        } else {
